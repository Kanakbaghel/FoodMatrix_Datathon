...
filter replaced the 50-country cap, the largest cleaned file
(`trade_matrix_cleaned.csv`) is about 84 MB — inside GitHub's limits, but treat
that as a happy accident rather than a guarantee. If a future config change
widens the item list, use Git LFS, a shared drive, or object storage instead.
